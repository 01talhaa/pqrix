    ...
    <>
      <main className="min-h-[100dvh] text-black dark:text-white">
        ...
        <section className="container mx-auto px-4 py-12 sm:py-16">
          <div className="grid gap-8 lg:grid-cols-2 lg:gap-12 items-start">
            ...
            <div className="order-2 lg:order-1">
              <div className="mb-4">
                <span className="inline-flex items-center rounded-full bg-green-100 dark:bg-lime-400/20 border border-green-300 dark:border-lime-400/30 px-4 py-1.5 text-sm font-medium text-green-600 dark:text-lime-400">
                  {member.department}
                </span>
              </div>
              <h1 className="mb-2 text-4xl font-extrabold tracking-tight sm:text-5xl md:text-6xl text-black dark:text-white">
                {member.name}
              </h1>
              <p className="text-xl font-medium text-green-600 dark:text-lime-400 mb-6">{member.role}</p>
              <p className="text-lg text-gray-700 dark:text-gray-300 mb-8">{member.fullBio || member.bio}</p>
              ...
              <div className="flex gap-3 mb-8">
                <Button
                  asChild
                  size="icon"
                  className="h-10 w-10 rounded-full bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-black dark:text-white"
                >
            ...
            <div className="order-1 lg:order-2">
              <div className="relative aspect-square rounded-2xl overflow-hidden liquid-glass border border-gray-200 dark:border-white/10">
                <img
            ...
            <Card className="liquid-glass border border-gray-200 dark:border-white/10 backdrop-blur-xl p-8">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {member.expertise.map((skill: any, idx: number) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-white/5 rounded-lg px-4 py-2"
                  >
                    <div className="h-2 w-2 rounded-full bg-green-500 dark:bg-lime-400" />
                    <span>{skill}</span>
                  </div>
                ))}
              </div>
            ...
            <Card className="liquid-glass border border-gray-200 dark:border-white/10 backdrop-blur-xl p-8">
              ...
              <div className="space-y-6">
                {member.experience.map((exp: any, idx: number) => (
                  <div key={idx} className="border-l-2 border-green-300 dark:border-lime-400/30 pl-6">
                    <h3 className="text-xl font-bold text-black dark:text-white mb-1">{exp.title}</h3>
                    <p className="text-green-600 dark:text-lime-400 font-medium mb-2">{exp.company}</p>
              ...
              <Card className="liquid-glass border border-gray-200 dark:border-white/10 backdrop-blur-xl p-8">
                ...
                <div className="space-y-4">
                  {member.education.map((edu: any, idx: number) => (
                    <div key={idx}>
                      <h3 className="text-lg font-bold text-black dark:text-white mb-1">{edu.degree}</h3>
                      <p className="text-green-600 dark:text-lime-400 mb-1">{edu.school}</p>
                ...
                <ul className="space-y-3">
                  {member.awards.map((award: any, idx: number) => (
                    <li key={idx} className="flex items-start gap-2 text-gray-700 dark:text-gray-300">
                      <div className="h-1.5 w-1.5 rounded-full bg-green-500 dark:bg-lime-400 mt-2 flex-shrink-0" />
                      <span>{award}</span>
                    </li>
                  ))}
                </ul>
                        ...
                        <Card className="liquid-glass border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/10 transition-colors overflow-hidden h-full">
                          <div className="relative aspect-video overflow-hidden">
                            {project.images && project.images.length > 0 ? (
                              ...
                              <div className="h-full w-full bg-gradient-to-br from-green-100 to-green-50 dark:from-lime-400/20 dark:to-cyan-400/20" />
                            )}
                          </div>
                          <div className="p-4">
                            <div className="flex items-center justify-between mb-2">
                              <span className="inline-flex items-center rounded-full bg-green-100 dark:bg-lime-400/20 border border-green-300 dark:border-lime-400/30 px-3 py-1 text-xs font-medium text-green-600 dark:text-lime-400">
                                {project.category}
                              </span>
                              <span className="text-xs text-gray-600 dark:text-gray-400">{project.status}</span>
                            </div>
                            ...
                            <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{project.description}</p>
                          </div>
                        </Card>
                      </Link>
                    ))}
        ...
        <section className="container mx-auto px-4 pb-16 sm:pb-24">
          <Card className="liquid-glass-enhanced border border-gray-200 dark:border-white/15 bg-gradient-to-br from-green-100 to-green-50 dark:bg-white/10 backdrop-blur-xl text-center p-8 sm:p-12">
            <h2 className="mb-4 text-3xl font-bold text-black dark:text-white sm:text-4xl">Work With Our Team</h2>
